...
> [!question] 한 기업이 **모든 멤버 계정의 비용 및 사용 보고서(CUR)를 한 달에 한 번 조회**해 청구 내역을 분석하려고 합니다. 확장성과 비용 효율이 가장 높은 솔루션은 무엇입니까?
> a) 관리 계정에서 CUR을 켜 **Amazon S3로 전달**하고 **Amazon Athena**로 분석한다
> b) 관리 계정에서 CUR을 켜 Kinesis로 전달하고 EMR로 분석한다
> c) 멤버 계정마다 CUR을 켜 S3로 전달하고 Redshift로 분석한다
> d) 멤버 계정마다 CUR을 켜 Kinesis로 전달하고 QuickSight로 분석한다
>> [!success]- Answer
>> a) 관리 계정에서 CUR을 켜 **Amazon S3로 전달**하고 **Amazon Athena**로 분석한다
>> **왜 이 답인가** — **관리 계정의 CUR에는 모든 멤버 계정 데이터가 포함**되고, S3에 쌓인 파일을 **한 달에 한 번만 Athena로 조회**하면 스캔한 만큼만 냅니다. 상시 클러스터가 없습니다.
>> **나머지가 아닌 이유** — EMR·Redshift는 클러스터를 운영해야 합니다. 계정마다 보고서를 켜면 관리가 흩어지고, Kinesis는 이 용도가 아닙니다.

<sub>관련: [[amazon-athena]] [[amazon-s3]] [[aws-organizations]] | 모듈 [[12-cost-compute]]</sub>
