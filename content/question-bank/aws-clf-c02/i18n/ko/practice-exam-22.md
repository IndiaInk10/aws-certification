...
> b) 컴퓨팅 사용량이 많은 워크로드를 위한 메모리 최적화 인스턴스
...
> b) Amazon S3 교차 리전 복제를 다른 AWS 리전으로 설정한다
...
> b) 결제 계정과 연결된 계정 모두가 멤버 계정을 조직에서 연결 해제하도록 요청하는 AWS Support 사례를 생성해야 한다.
...
> [!question] 한 회사가 리전 서비스 중단이 발생하는 경우 복구할 수 있게 해 주는 재해 복구 계획과 함께 AWS에서 고가용성 워크로드를 설정하려고 한다. 다음 중 이러한 요구 사항을 충족하는 구성은 무엇인가? (2개 선택)
