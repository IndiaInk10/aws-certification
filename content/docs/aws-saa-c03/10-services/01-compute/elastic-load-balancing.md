...
- ACM 인증서를 붙여 로드 밸런서에서 TLS 를 종료합니다. 백엔드까지 암호화하려면 대상 그룹도 HTTPS 로 둡니다
- **SNI** — 로드 밸런서 **하나에 인증서를 여러 장** 올려 도메인마다 다른 인증서를 씁니다. `여러 도메인을 로드 밸런서 하나로` 의 답이고, 도메인마다 ALB 를 만드는 보기와 대비됩니다. ALB·NLB 에서 되고 CLB 에서는 안 됩니다

**고정 세션(스티키 세션)**

- 쿠키로 같은 클라이언트를 **같은 대상에** 계속 보냅니다. 세션을 인스턴스 메모리에 두는 레거시 앱을 당장 굴릴 때 씁니다
- 대신 **부하가 한쪽으로 쏠립니다.** 근본 해법은 세션을 [[amazon-elasticache\|ElastiCache]]·DynamoDB 로 빼서 **상태 비저장**으로 만드는 것 — 시험은 대개 이쪽을 정답으로 봅니다

**교차 영역 로드 밸런싱**

| | 기본값 | 요금 |
|---|---|---|
| **ALB** | **켜져 있음** | 교차 AZ 전송 **무료** |
| **NLB · GWLB** | **꺼져 있음** | 켜면 교차 AZ 전송 **유료** |

- `AZ 마다 인스턴스 수가 달라 한쪽만 과부하` → **교차 영역 로드 밸런싱을 켜라**. NLB 에서 꺼져 있다는 것이 이 문항의 핵심입니다

**연결 드레이닝(등록 취소 지연)** — 인스턴스를 뺄 때 **처리 중인 요청이 끝날 때까지** 기다립니다. 기본 300초. `배포할 때 사용자 요청이 끊긴다` 의 답입니다.
...
- [ ] `여러 도메인을 로드 밸런서 하나로` → **SNI**
- [ ] `AZ 마다 대수가 달라 한쪽만 과부하` → **교차 영역 로드 밸런싱** (NLB 는 기본 꺼짐)
- [ ] `배포 중 요청이 끊긴다` → **연결 드레이닝**
- [ ] `세션이 유지돼야` → 고정 세션은 임시방편, 근본은 **세션 외부화**
