...
> [!important] 역할을 맡으면 원래 권한을 잃습니다
> **역할을 맡으면(AssumeRole)** 원래 갖고 있던 권한을 **내려놓고** 그 역할의 권한만 씁니다.
> **리소스 기반 정책**으로 접근을 받으면 **원래 권한을 그대로 유지한 채** 상대 자원까지 쓸 수 있습니다.
> `내 계정 자원과 상대 계정 자원을 동시에 다뤄야` 하는 지문에서 이 차이가 답을 가릅니다 — 역할을 맡는 보기로는 내 자원에 못 닿습니다.
...
- [ ] `양쪽 계정 자원을 동시에` → 역할이 아니라 **리소스 기반 정책**
- [ ] `안 쓰는 권한을 찾아내라` → **액세스 어드바이저 / 자격 증명 보고서**
