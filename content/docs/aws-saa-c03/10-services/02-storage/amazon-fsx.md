...
| | 성격 |
|---|---|
| **스크래치(Scratch)** | 임시. **복제하지 않아** 서버가 죽으면 데이터가 사라집니다. 대신 싸고 버스트 성능이 높습니다 — **짧은 처리 후 S3 로 내보내는** 워크플로 |
| **영구(Persistent)** | **AZ 안에서 복제**되고 장애 시 자동 복구합니다. 장기 저장·민감한 워크로드 |

- `처리하고 결과만 S3 에 남기면 된다` → 스크래치가 싸고 정답, `데이터를 잃으면 안 된다` → 영구입니다
