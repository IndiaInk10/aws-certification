...
| **Amazon MSK** | 관리형 **Apache Kafka**. `기존 Kafka 를 그대로` 가 지문에 있으면 Kinesis 가 아니라 MSK 입니다 |
...
- [ ] 처리량 부족 → **샤드 추가**(쓰기 1MB/s · 읽기 2MB/s) 또는 온디맨드
- [ ] `기존 Kafka 를 그대로` → **Amazon MSK**
- [ ] Flink 는 **Firehose 를 직접 못 읽습니다** — 앞에 Data Streams 가 필요
