...
**Chef와 Puppet**을 관리형으로 제공해, 서버 안쪽의 소프트웨어 구성을 코드로 관리하게 해 줍니다.

## 핵심 개념

- **구성 관리(Configuration management)** — 서버에 어떤 패키지가 깔려 있어야 하고 설정 파일이 어떤 상태여야 하는지를 코드로 정의하고 그 상태를 유지합니다.
- **Chef와 Puppet** — 업계에서 널리 쓰이던 구성 관리 도구입니다. 이미 이 도구로 레시피와 매니페스트를 쌓아 둔 조직이 AWS로 옮겨 올 때 그대로 쓰라고 만든 서비스입니다.
- **인프라가 아니라 서버 안쪽입니다** — 서버를 새로 만들어 내는 것이 아니라, 이미 있는 서버 **안의 소프트웨어 상태**를 다룬다는 점이 핵심입니다.
- **온프레미스 서버도 대상** — EC2 인스턴스뿐 아니라 데이터 센터의 서버까지 같은 방식으로 관리할 수 있습니다.
