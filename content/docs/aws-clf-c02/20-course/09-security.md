...
> [!tip] 정책 판정 규칙 두 가지
> **명시적 거부가 이깁니다** — 기본은 거부이고, 어떤 정책이든 허용하면 허용됩니다. 다만 **하나라도 명시적으로 `Deny` 하면 다른 정책이 아무리 허용해도 거부**됩니다. "허용과 거부가 함께 붙으면 어떻게 되나"는 단골 문제입니다.
> **IAM은 글로벌 서비스입니다** — 리전을 고르지 않습니다. 사용자·그룹·역할·정책은 모든 리전에서 그대로 쓰입니다.
...
> 한 가지 예외가 있습니다 — **SCP는 조직의 관리 계정에는 적용되지 않습니다.** 멤버 계정에만 걸립니다.
