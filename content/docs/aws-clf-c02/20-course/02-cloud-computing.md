...
| **절감형 플랜** | 시간당 **일정 금액**을 1년/3년 쓰겠다고 약정 | **최대 66~72%** | 1년 / 3년 | 예측 가능한 워크로드. 유연한 쪽(Compute)은 **인스턴스 패밀리·리전이 바뀌어도 적용**되고 그만큼 할인은 낮습니다 |
| **예약 인스턴스(RI)** | **특정 인스턴스**를 1년/3년 쓰겠다고 약정 | **최대 72%** | 1년 / 3년 | 사용량이 **꾸준하고 예측 가능한** 워크로드 |
...
여기에서 더 나아가 **전환형(Convertible) RI**를 고르시면 인스턴스 **패밀리·OS·테넌시**까지 바꿀 수 있지만, 그만큼 표준 RI보다 할인율이 낮습니다.
...
들어온 요청은 **정상 상태로 확인된 대상에만** 나눠 보냅니다.
...
- 온디맨드
- 스팟 인스턴스
> "3년"과 "예측 가능"이 함께 나오면 약정형입니다. 이 선택지 중에서는 **예약 인스턴스**가 최대 72% 절감으로 답이 됩니다. 전용 호스트는 격리·라이선스용이지 비용 최적화 수단이 아닙니다.
