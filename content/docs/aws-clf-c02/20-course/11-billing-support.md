...
> **들어오는(인바운드)** 데이터 전송은 무료이고, **같은 가용 영역 안**에서 주고받는 트래픽도 무료입니다.
> 다만 **가용 영역을 넘는 전송은 같은 리전 안이라도 요금이 붙습니다.**
...
> [!info] AWS가 최근 플랜 이름을 개편했지만, 시험은 위 4단계로 나옵니다
> **위 표와 응답 시간(24 · 12 · 4 · 1시간, 15분)을 그대로 외우시면 됩니다.** 지금 판매되는 플랜 구성이 다르다는 것만 알아 두세요.
