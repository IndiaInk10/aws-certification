...
| [[aws-trusted-advisor]] | 여러 범주로 계정을 점검하고 권장 사항을 준다 | 모범 사례 **자동 점검** |
...
> [!warning] 시험에 자주 나오는 두 가지
> **수집 주기** — EC2는 **기본 모니터링이 5분 간격**이고, **세부 모니터링을 켜면 1분 간격**이 됩니다. 세부 모니터링은 유료입니다.
> **메모리는 안 잡힙니다** — CPU·네트워크·디스크 I/O는 자동으로 수집되지만, **메모리 사용률과 디스크 사용량은 기본 지표에 없습니다.** 인스턴스 안쪽 값이라 **CloudWatch 에이전트를 설치**해야 수집됩니다. "메모리를 모니터링하려면?"이 나오면 에이전트입니다.
...
> **이번 강의에서 다룰 내용** — AWS 쪽 이벤트를 알려 주는 Health Dashboard와, 계정을 범주별로 점검해 주는 Trusted Advisor를 살펴보겠습니다.
...
| **운영 우수성**<br/>*(나중에 추가)* | 운영을 어렵게 만드는 구성 | — |

> [!warning] 범주가 5개인가요, 6개인가요
> **기출은 위 다섯 개**(비용 최적화·성능·보안·내결함성·서비스 한도)입니다. 이것을 외우시면 됩니다.
> 나중에 **운영 우수성**이 더해져 지금은 6개라는 것만 함께 알아 두시기 바랍니다.
...
| **AWS Trusted Advisor** | 비용·성능·보안·내결함성·서비스 한도(+운영 우수성) **범주 점검과 권장 사항** | "지속적인 평가", "모범 사례", "권장 사항", "여러 영역을 한꺼번에 개선" |
...
| **AWS OpsWorks**<br/>*(2024년 종료)* | Chef·Puppet 기반 **구성 관리** 관리형 서비스 | "Chef", "Puppet", "기존 구성 관리 도구를 그대로" |
...
- AWS Artifact
+ AWS Trusted Advisor
> **여러 범주를 한꺼번에** 검사하고 **권장 사항**을 준다는 점이 Trusted Advisor의 고유한 특징입니다. 비용 최적화·성능·보안·내결함성·서비스 한도(그리고 2023년에 더해진 운영 우수성)를 AWS 모범 사례에 비추어 자동으로 점검합니다. CloudWatch는 지표를 보여 줄 뿐 "이렇게 바꾸세요"라고 권장하지 않습니다.
...
- [ ] Trusted Advisor의 점검 범주를 나열할 수 있다 (기출 5개 + 나중에 더해진 운영 우수성)
