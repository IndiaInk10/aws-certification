...
| **Amazon QLDB** | 변경할 수 없는 **원장(ledger) 데이터베이스**. 모든 변경 이력을 암호화 검증이 가능한 형태로 남깁니다 | 일반 데이터베이스는 값을 덮어쓰지만, 원장은 **변경 이력이 지워지지 않습니다** |
...
| 지울 수 없는 변경 이력이 필요 | **Amazon QLDB** | "원장", "변경 불가", "암호화로 검증 가능한 이력", "감사 추적" |
