...
- **배포 사이 증분 다운로드** — 새 배포가 뜨면 **바뀐 화면만** 다시 받습니다. 근거는
  `public/offline-index.json` 으로, `next build` **뒤에** 도는 `scripts/build-sw.mjs` 가
  페이지마다 해시를 적어 내보냅니다. 해시는 이번 빌드가 실제로 내보내는 HTML·조각에서
  빌드 ID 만 지우고 계산합니다 — 빌드 ID 는 페이지마다 정확히 1회 등장하고, 같은 내용을
  두 번 빌드했을 때 달라지는 유일한 토큰입니다. 저장해 둔 해시와 같으면 HTML 이 바이트
  단위로 같다는 뜻이고, 그러면 그 HTML 이 부르는 `/_next/static` 주소도 이번 빌드에
  반드시 있습니다. 본문 한 줄 고친 배포 기준 327개 중 2개만 받습니다.
  인덱스를 못 받으면 옛 방식(전량)으로 떨어지므로 최악이 예전과 같습니다.
- **캐시 무효화** — 손댈 것이 없습니다. 통 이름에는 버전이 없고, 무엇을 버릴지는
  워커의 `reconcile` 이 위 해시로 판단합니다. `/_next/static` 은 이름이 곧 내용이라
  삭제 대신 **한 배포 유예를 둔 GC** 로 정리합니다 (열어 둔 탭이 옛 조각을 부를 수 있습니다).
